...
class GBXMLTestSuiteTest < MiniTest::Unit::TestCase
  ...
  def test_caseWBT2
    ...
    # If the argument has a default that you want to use, you don't need it in the hash
    args_hash = {}
    args_hash["testcases"] = "Whole Building Test Case 2"
    # using defaults values from measure.rb for other arguments
    ...
    # save the model to test output directory
    output_file_path = OpenStudio::Path.new(File.dirname(__FILE__) + "/output/test_case_wbt2/test_case_wbt2.osm")
    model.save(output_file_path,true)
	
	#save model as gbxml
	forwardTranslator = OpenStudio::GbXML::GbXMLForwardTranslator.new 
	output_file_path = OpenStudio::Path.new(File.dirname(__FILE__) + "/output/test_case_wbt2/test_case_wbt2.xml")
    forwardTranslator.modelToGbXML(model, output_file_path);
  end
end
